...
function cambioClave()
{
    ...
    header("Location: /Adman/Pantallas/profile.php");
    exit();
}
...
function DatosCarrera()
{
    ...
    $datosCarrera = $pdo->prepare($sql);
    $datosCarrera->execute();

    $rowCarrera = $datosCarrera->fetchAll(PDO::FETCH_ASSOC);
    return $rowCarrera;
}
...
function DatosMes()
{
    ...
    $datosMes = $pdo->prepare($sql);
    $datosMes->execute();

    $rowMateria = $datosMes->fetchAll(PDO::FETCH_ASSOC);
    return $rowMateria;
}
...
function InsertarDocumentos($AsuntoArchivoocumentacion) {
    ...
    $rutaBase = dirname(__DIR__, 2) . '/Instituto/';
    ...
    header("Location: /Adman/Pantallas/documentacion.php");
    exit();
}
...
function obtenerDatosParaGraficoCurso($year) {
    global $pdo;
    ...
    // Consulta SQL para obtener la cantidad de cada Carrera filtrada por el año
    $sql = "SELECT pl.Carrera, COUNT(*) as cantidad 
            FROM cuotaalumno ca 
            INNER JOIN plan pl ON pl.cod_Plan = ca.cod_Plan
            WHERE YEAR(ca.fecha_pago) = :year
            GROUP BY pl.Carrera";
    $stmt = $pdo->prepare($sql);
    ...
    if ($stmt->rowCount() > 0) {
        while ($fila = $stmt->fetch(PDO::FETCH_ASSOC)) {
            // Agregar datos a los arrays
            $datosCurso['Carrera'][] = $fila['Carrera'];
            $datosCurso['cantidades'][] = (int)$fila['cantidad'];
        }
    }

    // Convierte el array a formato JSON
    $datos_json_curso = json_encode($datosCurso);

    return $datos_json_curso;
}

function obtenerDatosParaGraficoCursoMes($yearCursoMes) {
    global $pdo;
    ...
    // Consulta SQL para obtener la cantidad de registros por mes para un año específico
    $sql = "
       SELECT 
        ...
        FROM cuotaalumno
        WHERE YEAR(fecha_pago) = :yearCursoMes
        GROUP BY YEAR(fecha_pago)
    ";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':yearCursoMes', $yearCursoMes, PDO::PARAM_INT);
    $stmt->execute();

    // Verificar si la consulta fue exitosa
    if ($stmt->rowCount() > 0) {
        $fila = $stmt->fetch(PDO::FETCH_ASSOC);
        foreach ($datosCursoMes['meses'] as $mes) {
            $datosCursoMes['cantidades'][] = (int)$fila[$mes];
        }
    }

    // Convierte el array a formato JSON
    $datos_json_curso_mes= json_encode($datosCursoMes);

    return $datos_json_curso_mes;
}
...
function insertarUsuario($user, $password, $plan, $rol, $estadoUser) {
    session_start();
    global $pdo;
    ...
    try {
        ...
        if ($stmtComprobarPlan->fetchColumn() > 0) {
            ...
            $sqlInsert = "INSERT INTO usuario (fk_DNI,User, Password, fk_Plan, fk_Rol, fk_Estado_Usuario) VALUES ( ?, ?, ?, ?, ?,?)";
            $stmtInsert = $pdo->prepare($sqlInsert);
            $dniUser = $_POST['dniUser']; // Asegúrate de que el campo esté presente en el formulario
            $stmtInsert->execute([$dniUser, $user, $password, $plan, $rol, $estadoUser]);
...
if (isset($_POST['btnmCrearUser'])) {
    ...
    insertarUsuario($user, $password, $plan, $rol, $estadoUser);
