...
function showConfirmationMessageUser($message, $rolUserEditar) {
  ...
  /*  if ($rolUserEditar == 3) {
        echo "window.location.href = '/Adman/lista_usuarios.php?rol=3';";
    } elseif ($rolUserEditar == 2) {
        echo "window.location.href = '/Adman/lista_usuarios.php?rol=2';";
    } elseif ($rolUserEditar == 1) {
        echo "window.location.href = '/Adman/lista_usuarios.php?rol=1';";
    } else {
        echo "window.location.href = '/Adman/lista_usuarios.php';";
    }*/
...
function showConfirmationMessagePlan($messagePlan) {
    echo "<script>
        Swal.fire({
            ...
            window.location.href = '/Adman/subPantallas/lista_planes.php';
...
function showConfirmationMessagesNotas($messageNota) {
    echo "<script>
        Swal.fire({
            ...
            window.location.href = '/Adman/Pantallas/Notas.php';
        });
    </script>";
}  

function showConfirmationMessagesPago($messagePago) {
    echo "<script>
        Swal.fire({
            icon: '" . $messagePago['type'] . "',
            title: '" . $messagePago['text'] . "',
            showConfirmButton: false,
            timer: 1500
        }).then(function() {                                  
            window.location.href = '/Adman/Pantallas/alumnos-cuotas.php';
        });
    </script>";
}  

function showConfirmationMessagesPagoActualizado($messagePagoActualizado) {
    echo "<script>
        Swal.fire({
            icon: '" . $messagePagoActualizado['type'] . "',
            title: '" . $messagePagoActualizado['text'] . "',
            showConfirmButton: false,
            timer: 1500
        }).then(function() {                                  
            window.location.href = '/Adman/Pantallas/alumnos-cuotas.php';
...
function editarUsuarios($claveeditaruser, $planUserEditar, $rolUserEditar, $UserId) {
    session_start();
    global $pdo;

    $sql = "UPDATE usuario SET Password = ?, fk_Plan = ?, fk_Rol = ? WHERE Id_Usuario = ?";
    $stmt = $pdo->prepare($sql);

    // Verifica si los valores son nulos antes de ejecutar la consulta
    $stmt->execute([$claveeditaruser,$planUserEditar, $rolUserEditar, $UserId]);
    ...
    if (isset($_GET['rol'])) {
        $rol = $_GET['rol'];
        header("Location: /Adman/subPantallas/lista_usuarios.php?rol=$rol");
    } else {
        header("Location: /Adman/subPantallas/lista_usuarios.php");
    }
    exit();
}
...
if (isset($_POST['btnmodificarUsuario'])) {
    ...
    editarUsuarios($claveeditaruser, $planUserEditar, $rolUserEditar, $UserId);
}
...
if (isset($_POST['btnmodificarPlan'])) {
    ...
    $folder = $_SERVER['DOCUMENT_ROOT'] . '/documentos/plan/'; // Ruta donde deseas almacenar los documentos
    ...
    if (!empty($nombreTarjeta) && !empty($estadoTarjeta) && !empty($fechaInicio) && !empty($fechaFinal)) {
        ...
        header("Location: /Adman/subPantallas/lista_planes.php");
        exit();
    } else {
...
////insertar 1 pago
function insertarNuevoPago($alumnoPago, $mes, $carrera, $montoCuota) {
    session_start();
    global $pdo;

    // Mapeo de los meses a las columnas correspondientes
    $meses = [
        1 => 'Enero',
        2 => 'Febrero',
        3 => 'Marzo',
    ...
    if (!array_key_exists($mes, $meses)) {
        $_SESSION['messagePago'] = [
            'type' => 'error',
            'text' => 'Mes inválido.'
        ];
        header("Location: /Adman/Pantallasalumnos-cuotas.php");
        exit();
    }
    ...
    $nombreMes = $meses[$mes];
     ...
     $sql = "INSERT INTO cuotaalumno (user, $nombreMes, cod_Plan, estado_pago, fecha_pago) 
     VALUES (?, ?, ?, 6, NOW()) 
     ON DUPLICATE KEY UPDATE $nombreMes = VALUES($nombreMes), cod_Plan = VALUES(cod_Plan), estado_pago = 6, fecha_pago = NOW()";

    $stmt = $pdo->prepare($sql);
    $stmt->execute([$alumnoPago, $montoCuota, $carrera]);
    ...
    if ($stmt->rowCount() > 0) {
        $_SESSION['messagePago'] = [
            'type' => 'success',
            'text' => 'Pago insertado exitosamente.'
        ];
    } else {
        $_SESSION['messagePago'] = [
            'type' => 'error',
            'text' => 'Ha ocurrido un error al insertar el pago.'
        ];
    }

    header("Location: /Adman/Pantallas/alumnos-cuotas.php");
    exit();
}


// Verificar si se ha enviado una solicitud de inserción
if (isset($_POST['btnmCrearPago'])) {
    $alumnoPago = $_POST['alumnoPago'];
    $mes = $_POST['mes'];
    $carrera = $_POST['carrera'];
    $montoCuota = $_POST['montoCuota'];
  

    insertarNuevoPago($alumnoPago, $mes, $carrera, $montoCuota);
    header("Location: /Adman/Pantallas/alumnos-cuotas.php");
    exit();
}

////// actualizar monto
function actualizarPago($id, $mesesData) {
    global $pdo;
    ...
    // Construir la consulta SQL dinámicamente
    $sql = "UPDATE cuotaalumno SET ";
    $params = [];
    foreach ($mesesData as $mes => $monto) {
        if (array_key_exists($mes, $meses)) {
            $sql .= "{$meses[$mes]} = ?, ";
            $params[] = $monto;
        } else {
    ...
    if ($stmt->rowCount() > 0) {
        $_SESSION['messagePagoActualizado'] = [
            'type' => 'success',
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id']) && isset($_POST['meses'])) {
    $id = $_POST['id'];
    $mesesData = $_POST['meses'];

    actualizarPago($id, $mesesData);
    header("Location: /Adman/Pantallas/alumnos-cuotas.php");
    exit();
}
...
function insertarNuevaNota($alumnoNota, $LegajoNota, $materia, $anioMateria, $estadoMateria, $parcial1, $recuperatorio1, $parcial2, $recuperatorio2, $finalnota, $promedio) {
    ...
    header("Location: /Adman/Pantallas/Notas.php");
    exit();
}
