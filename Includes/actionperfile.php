...
if (isset($_POST['token'])) {
	...
	if ($sql) {
		...
		header("location: /Adman/Pantallas/profile.php?success=$success");
	} else {
		// echo "error";
	}
		...
		if ($_POST['new_password'] == $_POST['confirm_new_password']) {
			...
			if ($p == $password) { //comprobamos que la contraseña sea igual ala anterior
				...
				if ($update_passwd) {
					...
					header("location:/Adman/Pantallas/profile.php?success_pass=$success_pass");
				}
			} else {
				$invalid = sha1(md5("la contraseña no coincide la contraseña con la anterior"));
				header("location: /Adman/Pantallas/profile.php?invalid=$invalid");
			}
		} else {
			$error = sha1(md5("las nuevas contraseñas no coinciden"));
			header("location: /Adman/Pantallas/profile.php?error=$error");
		}
	}
} else {
	header("Location: /Adman/Pantallas/profile.php");
